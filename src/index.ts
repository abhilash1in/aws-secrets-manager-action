...
let inputSecretNames: string[] = [...new Set(core.getInput(Inputs.SECRET_NAMES).split("\n").filter(x => x !== ""))]
// Check if any secret name contains a wildcard '*'
let hasWildcard: boolean = inputSecretNames.some(secretName => secretName.includes('*'))
...
const getSecretsManagerClient = (config) => new SecretsManager(config)
...
const getSecretValueMap = (secretsManagerClient: SecretsManager,
  secretName: string, shouldParseJSON: boolean = false): Promise<object> => {
